<#
.SYNOPSIS
Add or remove a DNS TXT record to deSEC.io
.DESCRIPTION
Note that this script is intended to be run via the dnsscript plugin from simple-acme.
...
deSEC.ps1 create {RecordName} {Token} DSToken DSTTL

deSEC.ps1 delete {RecordName} {Token} DSToken DSTTL
...
function Invoke-DeSEC {
    ...
    do {
        $retry = $false
        try { Invoke-RestMethod @queryParams }
        catch {
...
if ($Task -eq 'create'){
       Add-DnsTxt $RecordName $TxtValue $DSToken $DSTTL
}

if ($Task -eq 'delete'){
       Remove-DnsTxt $RecordName $TxtValue $DSToken $DSTTL
}
