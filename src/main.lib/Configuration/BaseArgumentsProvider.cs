...
namespace PKISharp.WACS.Configuration
{
        ...
        where T : class, IArguments, new()
    {
        ...
        private static void Configure(FluentCommandLineParser<T> parser)
        {
            ...
            {
                var setupMethod = typeof(FluentCommandLineParser<T>).GetMethod(nameof(parser.Setup), new[] { typeof(PropertyInfo) }) ?? throw new InvalidOperationException();
                ...
                var @as = clob.GetMethod(nameof(ICommandLineOptionBuilderFluent<object>.As), new[] { typeof(string) }) ?? throw new InvalidOperationException();
                ...
                if (!string.IsNullOrWhiteSpace(commandLineInfo?.Description))
                {
                    ...
                    var withDescription = clo.GetMethod(nameof(ICommandLineOptionFluent<object>.WithDescription), new[] { typeof(string) }) ?? throw new InvalidOperationException();
                ...
                {
                    var clo = typeof(ICommandLineOptionFluent<>).MakeGenericType(property.PropertyType);
                    var setDefault = clo.GetMethod(nameof(ICommandLineOptionFluent<object>.SetDefault), new[] { property.PropertyType }) ?? throw new InvalidOperationException();
        ...
        private T DefaultInstance
        {
            get
            {
                ...
                {
                    var type = typeof(T);
                    var constructor = type.GetConstructor(Array.Empty<Type>()) ?? throw new InvalidOperationException();
